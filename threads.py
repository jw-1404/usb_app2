...
class FileWriterThread(QThread):
    ...
    def stop(self):
        ...
        if self.collection_mode == "cosmic":
            command = ['online/bin/data_process.sh','-p','-i','120','-I','2','-r','1','-b',str(self.start_time),'-c','online/config','-d',self.file_path, '-L','0x7','-m','40','-k','3','-M','-K','tracks']
            proc=subprocess.Popen(command)
            proc.wait()
            command = ['online/bin/data_process.sh','-p','-i','120','-I','2','-r','1','-b',str(self.start_time),'-c','online/config','-d',self.file_path, '-L','0x7','-m','40','-k','2','-UDFPHM','-K','tracks_2']
