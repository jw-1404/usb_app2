...
class UsbReaderThread(QThread):
    data_received = pyqtSignal(bytes)
    error_occurred = pyqtSignal(str)
    finished_signal = pyqtSignal()

    def __init__(self, ep_in, timeout=60000):
        super().__init__()
        self.ep_in = ep_in
    ...
    def run(self):
        while self.running:
            try:
                with self.lock:
                    data = self.ep_in.read(16384, self.timeout)
                if data:
                    try:
                        self.queue.put_nowait(bytes(data))  
                        self.data_received.emit(bytes(data))
                    except queue.Full:
                        self.error_occurred.emit("队列满，丢弃数据包")  
                
                current_time = time.time()
                if current_time - self.last_queue_check >= 30:
                    queue_size = self.queue.qsize()
                    if queue_size > 15000:
                        self.error_occurred.emit(f"警告: 读取队列积压 {queue_size} 条数据，建议检查写入速度")
                    self.last_queue_check = current_time
                
            except usb.core.USBError as e:
                if e.errno != 110:

                    self.error_occurred.emit(f"读取线程错误: {e}")
                    break
...
class FileWriterThread(QThread):
    ...
    def __init__(self, queue, file_path, interval_minutes=0, start_time=None, collection_mode="baseline"):
        super().__init__()
        self.queue = queue
    ...
    def run(self):
        try:
            ...
            with open(self.file_path, "wb") as f:
                ...
                while self.running or not self.queue.empty():
                    ...
                    if len(self.batch_data) >= self.BATCH_SIZE:
                        f.write(self.batch_data)
                        self.batch_data.clear()
                        f.flush()

                    # 定时 flush
                    if time.time() - last_flush > 2 and self.batch_data:
                        f.write(self.batch_data)
                        self.batch_data.clear()
                        f.flush()
                        last_flush = time.time()
            ...
            self.error_occurred.emit(f"写入错误: {e}")
        finally:
            # 最后冲刺
            try:
                if self.batch_data:
    ...
    def stop(self):
        ...
        if self.collection_mode == "cosmic":
            command = ['online/bin/data_process.sh','-p','-i','120','-I','3','-r','-1','-b',str(self.start_time),'-c','online/config','-d',self.file_path, '-L','0x7','-m','40','-k','3','-M','-K','tracks']
            subprocess.Popen(command)
            command = ['online/bin/data_process.sh','-p','-i','120','-I','3','-r','-1','-b',str(self.start_time),'-c','online/config','-d',self.file_path, '-L','0x7','-m','40','-k','3','-UDFPHM','-K','tracks_2']
            subprocess.Popen(command)
...
class DrainDataThread(QThread):
    ...
    def __init__(self, ep_in, timeout=1000):
        super().__init__()
        self.ep_in = ep_in
        self.timeout = timeout
        self.running = True
        ...
        while self.running and attempts < max_attempts:
            try:
                ...
                if data:
                    ...
                    self.progress_signal.emit(f"排空数据: 已读取 {drained_bytes} 字节")
                    attempts = 0
                else:
                    attempts += 1
                    time.sleep(0.01)
                ...
                if e.errno == 110:
                    ...
                    self.progress_signal.emit(f"排空数据时发生错误: {e}")
                    break
            except Exception as e:
                self.progress_signal.emit(f"排空数据时发生未知错误: {e}")
                break
        self.progress_signal.emit(f"数据排空完成，总共读取 {drained_bytes} 字节")
        self.finished_signal.emit()
